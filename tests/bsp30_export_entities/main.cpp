...
int main()
{
    ...
    auto entities = BspEntities(bsp->GetRawEntityChars(), bsp->GetEntityCharCount());

#ifdef DECAY_JSON_LIB
    entities.ExportEntitiesJson("entities_raw.json");

    {
        nlohmann::json jEntities = entities.ExportEntitiesJson();
